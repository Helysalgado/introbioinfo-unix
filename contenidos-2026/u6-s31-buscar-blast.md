...
> **NOTA — Aula invertida.** **Antes de clase** lees este módulo y haces un primer intento: proponer,
...
> **IMPORTANTE — un acierto no es una respuesta.** La palabra *hit* suena a haber encontrado algo. Lo
...
> **NOTA — cómo lo hace BLAST en realidad.** La tabla usa coincidencia **exacta**, que es la versión
...
> **IMPORTANTE — BLAST es local, y ahora ya sabes lo que eso implica.** En la sección 7.4 de S30
...
> **ADVERTENCIA — BLAST no te dice lo que no encontró.** Cuando una búsqueda no devuelve nada, hay al
...
> **ADVERTENCIA — y no se pueden mezclar.** Pedirle a `blastn` que busque una proteína en una base de
...
> **IMPORTANTE.** El FASTA original sigue siendo el dato, y sigue viviendo en `data/source/` sin
...
> **TIP — por qué esta base y no otra.** No es un conjunto cualquiera: son **tres genes distintos en
...
> **NOTA — el paso 5 no es un tecnicismo.** Es una de las lecciones más útiles de la sesión: **la
...
> **ADVERTENCIA.** Este es el punto de la sesión donde más gente concluye de más. Si con palabra 7 no
...
> **TIP.** El error más grave no es el del programa. Es la última frase.
...
> **IMPORTANTE — la línea «Qué NO contiene» es la más valiosa de la ficha.** Es la que te va a
...
> **NOTA — el orden importa.** La figura 31.2 se enseña **después** de la Práctica 4, no antes. Si se
...
>
> **Origen de las cifras.** La tabla de tamaños de palabra de la sección 3 y la figura 31.2 se
> calcularon sobre los archivos reales, contando cuántas secuencias de la base comparten **al menos
> una palabra exacta** de longitud *k* con la consulta `ubiE_con.faa`:
>
> | *k* | `ubiE` que pasan | `era`+`hemE` que pasan |
> | ---: | ---: | ---: |
> | 3 | 19/19 | 38/38 |
> | 4 | 19/19 | 22/38 |
> | 5 | 19/19 | 1/38 |
...
> | 8 | **17/19** | 0/38 |
>
> Es coincidencia **exacta**, no la versión con umbral de vecindad que usa BLAST. El módulo lo
> declara explícitamente en la nota de la sección 3. La curva ilustra el compromiso; no predice el
> comportamiento literal de `blastp -word_size`.
>
> **Lo que la búsqueda debería encontrar.** Alineamiento local exacto (Smith–Waterman, BLOSUM62,
> apertura −11, extensión −1) de la consulta contra las 57 secuencias:
>
> | Posición | Puntuación | Familia |
> | --- | ---: | --- |
> | 1–19 | 543 – 1275 | **las 19 `ubiE`** |
> | 20 en adelante | ≤ 52 | `era` y `hemE` |
>
> El salto entre la posición 19 (543) y la 20 (52) es de **un orden de magnitud**. Es la razón por la
> que esta base funciona tan bien como ejercicio: la separación no es sutil. Dentro del bloque
> `ubiE`, el orden reproduce la distancia evolutiva —las cinco *Rickettsia* primero (1085–1275),
> después *Orientia* (650), después el resto de Rickettsiales (543–599)—.
>
> **Pendientes de validación en `chaac`** — antes de impartir:
>
> 1. `blastp -version`, para fijarla en el protocolo.
> 2. **Cuántos aciertos reporta realmente `blastp` con los valores por omisión.** Las `era` y `hemE`
>    tienen puntuaciones muy bajas, pero la base es pequeña y el umbral de significancia por omisión
>    es generoso: es posible que aparezcan algunas. **El módulo no afirma en ningún punto que no
>    aparecerán** —solo pide al estudiante que compruebe—, precisamente para que el material sea
>    correcto en cualquiera de los dos casos. Conviene ejecutarlo antes de clase y saber qué va a
>    salir.
> 3. El comportamiento de `-word_size 3/5/7` en `blastp` de la Práctica 4: comprobar si se pierde
>    algún acierto. **Si no se pierde ninguno, la práctica sigue siendo válida** —la pregunta 5 pide
>    razonar sobre cómo te habrías dado cuenta— pero conviene saberlo para conducir la discusión.
>    Nota: `blastp` admite tamaños de palabra pequeños; valores altos pueden requerir ajustar otros
>    parámetros o ser rechazados según la versión.
> 4. El mensaje literal de error de la Práctica 5, paso 4 (`blastn` contra una base de proteínas).
> 5. Que `sseqid` salga como `ID:WP_010977615.1`. Los encabezados tienen un **espacio justo después
>    del identificador**, así que el primer campo es limpio y único —se comprobó que los 57 son
>    distintos—. Si una versión de `makeblastdb` recortara de otra forma, hay que ajustar el `grep`
>    del paso 5 de la Práctica 3.
>
> **Decisiones de alcance:**
>
> - **No se interpretan `pident`, `evalue` ni `bitscore`.** Se nombran las columnas 1, 2 y 12 y se
>   declara explícitamente que las otras nueve son S32. Es la restricción más importante del prompt
>   de la sesión y sostiene la tensión narrativa hacia el módulo siguiente.
> - **No se introduce `blastx` ni `tblastn`.** Se mencionan como familia sin desarrollarlas.
> - **Comandos nuevos de Unix: ninguno**, salvo `comm` en la Práctica 4, paso 3. Si no se cubrió en
>   U4, sustituir por `sort` + `diff`, o resolverlo visualmente: las listas son de 19 líneas.
> - **El ciclo `for` de la Práctica 4 es el de S26**, sin variantes nuevas. Es una reaparición
>   deliberada: el estudiante debería reconocerlo sin explicación.
> - **No se usa `qsub`.** Estas búsquedas son de segundos. La infraestructura de S29 vuelve a ser
>   pertinente en S33, si el informe final requiere una base grande.
> - Sigue prohibido el vocabulario de homología en las entregas. La interpretación de métricas se
>   levanta en S32; homología / ortología / paralogía, en S33.
>
> **Preparación previa del taller:** comprobar que `makeblastdb`, `blastp` y `blastn` están en el
> `PATH` de las cuentas, y ejecutar una vez la Práctica 3 completa para conocer la salida real.