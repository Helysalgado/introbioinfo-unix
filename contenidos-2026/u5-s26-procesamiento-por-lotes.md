...
> **NOTA — Aula invertida:** Antes de clase harás un primer intento sin ejecutar nada: organizar la
...
> **IMPORTANTE — hoy no aprendes un ciclo.** Aprendes a **ejecutar el mismo experimento sobre una
...
> **TIP — de dónde sale la colección.** Puede ser el conjunto del mini proyecto, un grupo de
...
> **NOTA — cómo están marcadas las ideas.** Igual que en S24 y S25: **Concepto esencial**,
> **Concepto de apoyo** y **Consulta**.

> **IMPORTANTE — qué cabe en dos horas.** En el taller se construye el recorrido, se comprueba que
...
> **ADVERTENCIA — la segunda decisión tiene una consecuencia que descubrirás en el taller.** Si todos
...
> **COMENTARIO — el criterio de selección es una decisión científica.** Un conjunto de doce bacterias
...
> **NOTA — el patrón lo expande el shell, no el ciclo.** Antes de empezar a dar vueltas, el shell
...
> **ADVERTENCIA — si el patrón no encuentra nada, el ciclo se ejecuta igual.** Con una vuelta, y con
...
> **COMENTARIO — un cambio menor y necesario en la herramienta.** Hay una sola cosa que la herramienta
...
> **ADVERTENCIA — `$?` guarda el resultado del comando *inmediatamente* anterior.** Si escribes un
...
> **NOTA — es la práctica más larga de la unidad, y va por partes.** Cada una termina en algo que
...
> **NOTA:** Trabaja sobre una copia: `cp src/procesar-lote.sh /tmp/prueba-lote.sh`.
...
> **IMPORTANTE — el apartado 4 es el que convierte el lote en un experimento.** Sin bitácora no puedes
...
> **ADVERTENCIA:** Un asistente tenderá a proponerte paralelizar el lote —ejecutar varios organismos a
...
> **TIP:** Antes de S27, haz este experimento y anota el resultado: entrega tu proyecto a un compañero
...
> `Plan-Clases-BioInfo-2026-final-S34.xlsx`, pestaña **`PlanClases-2026-final S34`**, fila S26:
> *De un genoma a una colección: análisis por lotes*, Comp. E, ajuste **[Ajustado]**, evidencia
> *«lote de genomas procesado con resultados separados»*. **Sin discrepancias.**
>
> Cuatro precisiones:
>
> - El Plan menciona «registro básico de ejecución»: aquí se concreta como la **bitácora del lote**
>   (Sección 4.3), que es también lo que da sentido retroactivo al `exit 1` de S25.
> - El Plan lista el clúster institucional entre los recursos de S26. Este módulo **no** lo usa: la
>   ejecución en clúster se trabaja en S29, y aquí se nombra únicamente al desaconsejar la
>   paralelización (Cierre con IA). Conviene que así conste, para no duplicar contenido.
> - **La lectura base de la unidad (Buffalo, Cap. 12) entrega su evidencia en esta sesión**, conforme
>   a lo fijado en la portada.
> - **Requisito material:** la sesión necesita una **colección de al menos cuatro organismos** por
>   estudiante o equipo, organizada antes del taller. Conviene tener un conjunto de doce preparado en
>   el servidor y anunciarlo al cerrar S25.
>
> **Ajuste propuesto respecto a la arquitectura.** La arquitectura de unidad preveía que la
> herramienta de S25 no se tocara en S26. Aquí se le añade **un** parámetro —el directorio de salida—
> porque, sin él, todos los organismos escribirían en el mismo sitio. El cambio es de una línea, se
> presenta como consecuencia natural de la lección de S25 (quien invoca decide) y no altera el
> análisis. Queda registrado por si se prefiere resolverlo de otro modo.
>
