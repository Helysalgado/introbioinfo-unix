...
> **NOTA — Aula invertida:** Antes de clase harás un primer intento sin ejecutar nada: auditar tu
...
> **IMPORTANTE — otra vez, ninguna herramienta de análisis nueva.** Sigues sin aprender nada nuevo
...
> **TIP — qué segundo genoma elegir.** Sirve cualquier bacteria con FASTA y GFF3 en el mismo recurso
...
> **NOTA — cómo están marcadas las ideas dentro de cada sección.** Igual que en S24:
...
> **IMPORTANTE — qué cabe realmente en dos horas.** En el taller se transforma el script y se le
...
> **ADVERTENCIA:** Este error es especialmente peligroso porque **es intermitente**. Tu script
...
> **TIP:** `basename` recorta la ruta y, si le das una extensión, también la quita. Pruébalo suelto en
...
> **COMENTARIO — tu script acaba de adquirir una forma oficial de usarse.** Desde hoy no basta con
...
> **COMENTARIO — por qué esto importa más de lo que parece.** No es orden por orden. Un resultado sin
...
> **NOTA — `-f` y `-s` no son lo mismo, y la diferencia te va a salvar.** `-f` solo dice que el
...
> **TIP — los espacios de los corchetes no son decorativos.** `[` es un comando, así que necesita
...
> **IMPORTANTE — el apartado 5 es el que convierte esto en trabajo científico.** Un script cuyas
...
> **ADVERTENCIA:** El riesgo de hoy no es que te dé código malo: es que te dé código **bueno de más**.
...
> **TIP:** Guarda tu herramienta tal como quedó hoy. En S26 no la vas a modificar apenas: la vas a
...
> `Plan-Clases-BioInfo-2026-final-S34.xlsx`, pestaña **`PlanClases-2026-final S34`**, fila S25:
> *Separar el procedimiento de sus datos*, Comp. E, ajuste **[Ajustado]**, evidencia *«script
> parametrizado y validado»*. **Sin discrepancias.**
>
> Tres precisiones sobre la fila del Plan:
>
> - El Plan lista «mensajes de uso; validación de archivos y directorios; decisiones con `if` y
>   códigos de salida». Se cubre todo; la comprobación de **directorios** (`-d`) se resuelve creando
>   los de salida con `mkdir -p`, que es lo que el flujo necesita, en vez de comprobar su existencia.
> - El título del Plan nombra las construcciones; aquí se titula por lo que resuelven, conforme a la
>   regla editorial del `README.md`. El contenido es el del Plan, íntegro.
> - **Requisito material:** la sesión necesita un **segundo genoma** por estudiante o equipo,
>   descargado y verificado antes del taller. Conviene anunciarlo al cerrar S24 y, si es posible,
>   dejar preparado un conjunto de respaldo en el servidor para quien no lo consiga a tiempo.
>
