# Unidad 2 · Módulo S3 — Shell, acceso remoto y transferencia

> **NOTA — Lectura previa (aula invertida).** Este documento se lee **antes de la sesión S3**. Aquí
...
> sesión*) y tus dudas. A lo largo del módulo hay **cinco prácticas** y un **reto integrador**.

Primer módulo de la [Unidad 2](u2-entorno-unix-hpc.md). Su meta es que, partiendo de un problema real
...
conectarte por SSH, moverte lo mínimo dentro del servidor y transferir archivos verificando su
integridad.
...
| **Evidencia** | Registro reproducible de una transferencia cuya integridad se comprobó |

## Relación con la Unidad 1 y con el proyecto integrador

En la Unidad 1 trabajaste con `pacientes.md`, un conjunto de datos **sintético** de tres registros, y
elaboraste sus **metadatos**. También aprendiste dos ideas que hoy usaremos: los **datos originales se
conservan sin modificaciones**, y deben ir **acompañados de la información que permite
interpretarlos**. A partir de esta unidad, el trabajo computacional se realiza en el **servidor del
curso**. El primer paso es llevar allí tu archivo de datos y sus metadatos, comprobar que llegaron
intactos y dejar registrado el procedimiento para poder repetirlo.
...
1. **Explicar** por qué la bioinformática usa Unix y la línea de comandos, en términos de
   reproducibilidad.
2. **Identificar** las partes de un comando y **consultar** la ayuda del sistema (`man`, Tab,
   historial, `Ctrl-C`).
...
4. **Transferir** un archivo y sus metadatos con SFTP o FileZilla, distinguiendo el lado local del
   remoto.
5. **Comprobar** la integridad de la transferencia con *checksums* y **documentar** todo de forma
   reproducible, sin credenciales.

## Antes de la sesión

> **IMPORTANTE — credenciales.** La **dirección del servidor**, tu **usuario** y tu **contraseña** se
...
| **Primer intento (preparación)** | Los seis puntos del preflight (abajo). |
| **Producto para el taller** | `pacientes.md` y `pacientes-metadatos.md` listos, el entorno probado y al menos una duda anotada. |
...
- [ ] **Ten los dos archivos a la mano** para transferirlos en clase.

> **ADVERTENCIA — seguridad, léela antes de la sesión.** Nunca compartas contraseñas, llaves privadas
...
![Comparación entre una interfaz gráfica operada con el ratón y una interfaz de línea de comandos operada con texto reproducible.](images/figura-u2-gui-vs-cli.png)
...
![Cuatro herramientas de Unix (grep, cut, sort y uniq -c) conectadas mediante tuberías para formar un flujo de análisis que cuenta ocurrencias por categoría.](images/figura-u2-filosofia-unix.png)
...
# ejemplo:
ls  -l  /ruta/a/una/carpeta
```

- **comando:** la herramienta (`ls`, `cd`, `cp`…).
...
![Descomposición de un comando en sus tres partes: comando, opción y argumento.](images/figura-u2-anatomia-comando.png)
...
> **NOTA:** "Seguro" en SSH significa que la comunicación viaja **cifrada**: aunque alguien intercepte
> los datos, no puede leerlos. Por eso para nuestros datos preferimos **SFTP** (sobre SSH) y no FTP
> plano.

> **Comprobación conceptual (rápida, para el taller).** Sin ejecutar nada todavía, intenta responder:
> (a) en `ls -l pacientes.md`, ¿cuál es el comando, la opción y el argumento?; (b) ¿qué diferencia hay
> entre la terminal y el shell?; (c) para mover un archivo de datos de forma segura, ¿elegirías FTP o
> SFTP, y por qué?; (d) ¿por qué un comando de texto favorece la reproducibilidad?
...
> **NOTA — la contraseña no se ve mientras la escribes.** Al teclear la contraseña, SSH **no** muestra
...
![Diagrama de una conexión SSH: la computadora del estudiante como cliente se conecta de forma cifrada a través de internet con el servidor remoto.](images/figura-u2-conexion-ssh.png)

*Figura 4. Conexión por SSH: tu computadora (cliente) establece un canal cifrado por internet hacia el servidor remoto.*

> **ADVERTENCIA:** Si algún día SSH te avisa de que la huella **cambió** inesperadamente, no la
> aceptes: detente y consúltalo. Un cambio no anunciado puede indicar un problema de seguridad.

> **¿SABÍAS QUE?:** Los servidores de cómputo suelen llevar nombres temáticos (deidades, ríos,
...
> **IMPORTANTE:** Nunca ejecutes un comando de transferencia sin tener claro **desde qué contexto** lo
...
> **NOTA:** `history` te ayuda a **reconstruir** lo ejecutado, pero **no** sustituye una bitácora
...
> **NOTA:** No necesitas que `pacientes.md` ya esté en el servidor para practicar Tab; usa cualquier
...
Si usas **SFTP por terminal**, este es el conjunto mínimo:

```text
[SFTP] Dentro de la sesión de transferencia (prompt sftp>):
lpwd     # (local)  ¿en qué carpeta local estoy?
lls      # (local)  lista archivos de tu computadora
pwd      # (remoto) ¿en qué carpeta del servidor estoy?
ls       # (remoto) lista archivos del servidor
put      # sube un archivo (local -> servidor)
get      # descarga un archivo (servidor -> local)
exit     # cierra la sesión SFTP
```

- `lpwd`, `lls`, `lcd`: actúan sobre el **lado local**.
- `pwd`, `ls`, `cd`: actúan sobre el **lado remoto**.
- `put`: **subir**; `get`: **descargar**; `exit`: cerrar SFTP.

> **NOTA:** Aquí no profundizamos en navegación ni rutas: eso es tema de **S4**. Por ahora usa tu
> directorio inicial en el servidor (o la carpeta que indique el docente) como destino.
...
![Esquema didáctico de FileZilla con dos paneles: a la izquierda el sitio local y a la derecha el sitio remoto, con la barra de conexión arriba.](images/figura-u2-filezilla-esquema.png)

*Figura 5. Esquema didáctico de FileZilla (no es una captura real): panel local a la izquierda, panel remoto a la derecha; los archivos se arrastran entre ambos.*

> **NOTA — adelanto de S4.** Existen también `scp` y `rsync` para transferir desde la línea de comandos
...
> | --- | --- | --- |
> | **SFTP / FileZilla** | Subir/bajar archivos puntuales de forma guiada e interactiva | **S3 (esta sesión)** |
...
8. **Confirma** con `ls -lh` que ambos archivos llegaron.
...
```bash
[LOCAL o REMOTO] En Linux (y en el servidor):
...
- ambos comandos usan **SHA-256** (mismo algoritmo, distinto nombre según el sistema);
...
> **Después de la práctica, responde:** (1) ¿Coincidieron las huellas de origen y destino? (2) ¿Por qué
> no basta con ver que el archivo "está" en el servidor?

---

## Reto integrador S3 — Trasladar un dato científico de forma segura y reproducible

Este reto **no** es la primera vez que haces estas acciones: es integrarlas **sin instrucciones paso a
paso**. Trabájalo de forma individual o en parejas.

1. Conéctate por SSH.
2. Verifica que estás en el servidor correcto (`hostname`).
3. Reconoce tu usuario y tu ubicación (`whoami`, `pwd`).
4. Sal (`exit`).
5. Transfiere `pacientes.md` y `pacientes-metadatos.md` por SFTP/FileZilla.
6. Vuelve a conectarte por SSH.
7. Confirma que ambos archivos llegaron (`ls -lh`).
8. Calcula y **compara** los *checksums* de origen y destino.
9. Consulta `man` si necesitas recordar una opción.
10. Cierra correctamente las sesiones.
11. **Documenta** el procedimiento en tu bitácora/protocolo.

> **Recuerda — cierre y enlace con S4.** En S3 trasladamos al servidor `pacientes.md` y sus metadatos.
> En **S4** construiremos la **estructura reproducible del proyecto** y colocaremos el original intacto
> en `data/source/`.

## Evidencia de aprendizaje

Un **registro reproducible** de la transferencia que incluya:

- el **objetivo** de la transferencia;
- el **sistema local** usado (macOS, Linux o Windows con su cliente);
- el **host** del servidor, **sin** contraseña;
- la **distinción** entre acciones `[LOCAL]`, `[REMOTO]` y `[SFTP]`;
- los **comandos exactos** ejecutados;
- el registro de **al menos una conexión y salida** correctas;
- una **opción de `ls`** localizada mediante `man`;
...
> **NOTA:** La evidencia **no** debe limitarse a una captura de pantalla; el valor está en el registro
> reproducible.
...
| Transferencia con metadatos | Sube dato **y** metadatos distinguiendo local/remoto | Sube solo el dato | No transfiere |
| Verificación de integridad | Calcula y **compara** checksums; coinciden | Calcula sin comparar | No verifica |
| Registro reproducible | Comandos, contextos y checksums, sin credenciales | Registro incompleto | Sin registro o con credenciales |

## Autoevaluación — semáforo de salida

- 🟢 **Verde:** entré y salí del servidor varias veces, transferí el dato y sus metadatos y mis
  checksums coincidieron.
...
| 25–40 min | Comandos y ayuda dentro del servidor (Práctica 3) |
| 40–55 min | Tres ciclos de conexión, reconocimiento y salida (Práctica 2) |
...
Elementos operativos que dependen de la infraestructura y conviene confirmar (y entregar al grupo por
un canal seguro) antes de la sesión:

- **Host** (dirección del servidor) y si se entrega en clase o con anticipación.
- **Huella oficial** (SHA-256) del servidor para verificar la identidad.
- **Puerto** de SSH/SFTP (normalmente 22, confirmar).
- **VPN** o red requerida fuera del campus.
- **Directorio inicial** o carpeta de destino asignada para la transferencia en S3.
- **Disponibilidad de credenciales** antes de la sesión (define si se pide o no una conexión previa).
