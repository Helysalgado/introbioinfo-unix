# Unidad 4 · Sesión 18 — Precisar: decir exactamente lo que se quiere buscar
...
> **IMPORTANTE:** Esta sesión no continúa el mini proyecto ni el examen práctico. Continúa
...
> **NOTA:** Fíjate en que hoy casi no aparecen preguntas nuevas. Esta es una sesión de
...
> **TIP:** Si perdiste `results/s13/`, puedes regenerarlo, pero anótalo en el protocolo. Un número
...
> **NOTA — sobre la última columna de la figura.** El patrón `^(?!pseudo)gene$` que aparece ahí
> pertenece a un dialecto más rico (PCRE) y **`grep -E` no lo acepta**. En esta sesión conseguirás el
> mismo efecto anclando el patrón, que además es más legible. Lo verás en la Sección 3.

> **IMPORTANTE:** Que un programa produzca una salida no significa que esa salida sea correcta. Una
> búsqueda mal especificada no devuelve un error: devuelve un número perfectamente razonable, con la
> magnitud esperada, en el formato esperado. **Las consultas también se validan.** Ese es el mensaje
...
> **¿SABÍAS QUE?:** Las expresiones regulares provienen del trabajo del matemático Stephen Kleene en
...
> expression and print*). El asterisco que usarás hoy se sigue llamando "estrella de Kleene".

### 2.2 Laboratorio de visualización de patrones

Antes de ejecutar nada sobre tus archivos, conviene **ver** cómo se comporta un patrón. Para eso
existen visualizadores interactivos: escribes un patrón arriba, un conjunto de cadenas de prueba
abajo, y el sitio resalta qué parte de cada cadena coincide.

Usaremos dos visualizadores:
...
> **ADVERTENCIA:** El visualizador sirve para **comprender y depurar** un patrón. La evidencia
> científica se obtiene siempre ejecutando el comando sobre los archivos del proyecto y quedando
> registrada en el protocolo. Una captura de pantalla del visualizador no es un resultado. Y ten en
> cuenta que estos sitios usan por defecto sabores de expresión regular más ricos que el de
> `grep -E`: lo que allí funciona debe comprobarse siempre en la terminal.

Verás la anatomía de estas herramientas y el uso concreto que les daremos en la **Figura 18.6**,
justo antes de la Práctica 2.
...
> **TIP — usa siempre comillas simples.** El patrón lo interpreta `grep`, no el shell. Si escribes
...
> **ADVERTENCIA:** El punto sin escapar es la causa silenciosa de falsos positivos más frecuente
...
variable —un `locus_tag` como `b0001` es una letra seguida de varios dígitos— y porque describir "uno
o más" es la única forma de aceptar todos los casos sin enumerarlos.

> **IMPORTANTE — `*` no significa lo mismo que en el shell.** En S4 aprendiste que `*.fna` significa
...
> **TIP:** Cuando un patrón con alternancia te dé un número raro, mira primero los paréntesis. La
...
> **IMPORTANTE:** Esta es la razón por la que `-w` no bastaba. Elimina las coincidencias dentro de
...
> **IMPORTANTE:** La precisión forma parte del pensamiento científico, no de la informática. Un
...
8. **Documenta.** Todo esto va a la tabla de refinamiento del protocolo.
...
> **NOTA — el tabulador dentro del patrón.** `$'...'` es una forma de comillas del shell que
...
Agrega a `doc/protocolo.md`, después de la sección de S13, la sección de hoy. **No modifiques las
secciones anteriores**: la comparación entre las tres versiones —S12, S13, S18— es la evidencia de
que tu análisis mejoró.
...
> **IMPORTANTE:** No presentes esta sección como una lista de errores corregidos. Es la **evolución
...
## Evidencia de aprendizaje de S18
...
| Probar el patrón solo en el visualizador | El sitio usa un sabor de regex más rico que `grep -E` | Reejecutar siempre en la terminal antes de documentar |
...
> **ADVERTENCIA:** Los asistentes tienden a proponer patrones de sabores más ricos (PCRE) que
> `grep -E` no acepta, como `\d` o `\b`. Si un patrón sugerido no funciona en tu terminal, no es
> necesariamente un error del asistente: puede ser un patrón correcto en otro dialecto. Comprueba
> siempre en la terminal y anota la discrepancia.
...
habías escrito y lo corregiste, dejando constancia de cuánto se equivocaba**. Esa es la diferencia
entre un cuaderno de laboratorio y un informe. Un análisis reproducible no es el que nunca se
equivoca: es aquel en el que se puede saber **en qué** se equivocaba y **por qué** la versión nueva
es mejor.

Si tuvieras que resumir la sesión en una frase, no sería "aprendí expresiones regulares". Sería
*"aprendí a comprobar que mi consulta pide lo que yo creía que pedía"*.
...
> **TIP:** Guarda `results/s18/` completo y conserva `results/s13/` sin tocar. En S19 necesitarás las
> líneas correctamente seleccionadas de hoy como punto de partida, y la comparación entre las tres
> versiones de tus conteos —S12, S13, S18— es una de las evidencias más valiosas de tu protocolo.
