...
> **NOTA:** El **cómo** hacer bien estas fases se detalla más adelante: la **organización del
> proyecto** (fase de conservación) en la sección 6, y los **metadatos y principios FAIR** (fases de
> registro de procedencia y documentación) en la sección 7.
...
*Figura 4. Componentes principales de StackEdit: (1) panel de edición, (2) vista previa, (3) barra de herramientas, (4) menú lateral y (5) explorador de documentos. Captura propia de StackEdit (stackedit.io).*
...
*Figura 5. Organización de un proyecto reproducible: separa los datos (originales en `data/source/` y derivados en `data/processed/`) del código, los resultados y la documentación (Noble, 2009).*
...
Esta sección no es un tema aparte: responde al **cómo** de varias fases del manejo de datos (proceso
A, sección 2). Cuando **registras la procedencia** (fase A2), **conservas** los datos (fase A6) y
**documentas** todo (fase A7), lo que en realidad estás haciendo es aplicar **metadatos** y
**principios FAIR**. Y esos metadatos no viven en el aire: se guardan junto a los datos originales en
`data/source/`, dentro de la estructura de proyecto de la sección 6.
...
*Figura 6. Los principios FAIR para datos y software. Son cuatro condiciones complementarias para que un recurso científico conserve su valor y pueda reutilizarse; FAIR no significa necesariamente "abierto" o "gratuito".*
...
Para que no queden como principios abstractos, esta tabla muestra **en qué fase del manejo de datos
(proceso A, sección 2) aplicas cada principio** y dónde se refleja en tu proyecto:
...
> **COMENTARIO:** Dicho en breve: los metadatos y FAIR son **cómo se hacen bien** las fases de
> *registro de procedencia*, *conservación* y *documentación* del proceso A. No son un paso extra,
> sino la calidad con la que ejecutas esos pasos.
...
*Figura 7. Los cuatro bloques esenciales de un prompt científico —contexto, objetivo, formato y verificación— con un ejemplo integrado. La lista de arriba los desarrolla en detalle.*
...
*Figura 8. Antes de usar una respuesta de IA hay que entenderla, probarla y contrastarla. Si no es confiable, se corrige y se vuelve a validar; la responsabilidad del resultado es siempre de quien realiza el análisis.*
...
**Antes de clase (primer intento).** Trabajarás con un conjunto de datos pequeño del curso,
`ejemplos/pacientes.md` (datos biométricos anónimos de pacientes, con fines educativos).

1. **Dibuja o escribe como texto** la estructura de directorios de la sección 6 para un proyecto que
   analice esos datos (no necesitas comandos de Unix). Ubica `pacientes.md` dentro de `data/source/`.
2. Redacta un borrador de **metadatos** para ese archivo con los campos **(mínimo U1)** de la
   sección 7.1. Usa como **modelo** el archivo `ejemplos/metadatos_pacientes.md`, que incluye una
   descripción, la información del archivo y un **diccionario de variables** (qué significa cada
   columna: `id`, `peso`, `altura`, `sexo`, `edad`, `dx`).

**Durante el taller.** Compararemos estructuras, discutiremos qué va en `data/source/` vs.
`data/processed/`, revisaremos el diccionario de variables y completaremos los metadatos faltantes.

**Después del taller (entrega final).** Entrega el árbol del proyecto (dibujado o en texto) y el
archivo de metadatos de `pacientes.md` con los campos mínimos completos y su diccionario de variables.
