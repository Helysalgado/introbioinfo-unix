# Unidad 3 · Sesión 8 — Bases de datos, descarga y verificación de integridad
...
| **Propósito** | Recuperar un conjunto de archivos coherente de un ensamblado real y verificar que la descarga es íntegra y reproducible |
...
> **ADVERTENCIA:** NCBI no es un tema de esta sesión; es un ejemplo. Lo que aprendes aquí —elegir un
...
6. **Verificar** la integridad de una descarga comparando un checksum publicado con uno calculado
   (a nivel conceptual; el comando se ejecuta en S9).
...
```text
objeto biológico → registro → base de datos → formato (FASTA, GFF3, GenBank)
```

Un **registro** es la entrada de una base de datos que reúne, para una entidad concreta —una
secuencia, un ensamblado, una publicación—, sus campos, su identificador y, casi siempre, más de una
forma de descargarlo. Una **base de datos biológica** es el sistema que organiza, versiona y expone
esos registros de forma consultable. El registro no es el objeto biológico —sigue siendo una
representación—, pero es el punto de partida obligado para llegar a un archivo con procedencia
conocida.

![](images/figura-u3-reproducibilidad.png)
...
> **IMPORTANTE:** Empezar por "voy a buscar en NCBI" salta directamente a la herramienta. El orden
...
| Base de datos | Qué contiene | Cuándo la consultas |
| --- | --- | --- |
| **Assembly** | Ensamblados genómicos completos, con su nivel de organización y versión | Cuando necesitas el genoma completo de un organismo, cepa o aislamiento |
| **Genome** | Panorama de los ensamblados disponibles por organismo | Cuando quieres comparar cuántos ensamblados existen para una especie |
| **Nucleotide** | Registros de secuencias de ácidos nucleicos individuales (no siempre genomas completos) | Cuando buscas una secuencia concreta, como un gen o un cromosoma específico |
| **Gene** | Información curada sobre un gen: función, ubicación, sinónimos, referencias | Cuando necesitas contexto biológico de un gen, más allá de su secuencia |
| **Protein** | Secuencias y anotaciones de proteínas | Cuando la evidencia que necesitas es la secuencia o función de una proteína |
| **Taxonomy** | Clasificación taxonómica y nombres científicos | Cuando necesitas confirmar el organismo exacto o su lugar en la clasificación |
| **PubMed** | Registros bibliográficos de artículos científicos | Cuando necesitas la publicación que respalda un registro (el campo `REFERENCE` que viste en GenBank, S7) |


> **TIP:** Puedes explorar el ecosistema completo desde un solo punto de entrada:
...
llegar a un registro concreto, la cadena de decisiones es:

```text
pregunta biológica → organismo → cepa o aislamiento → ensamblado → versión → registro
```

Cada eslabón descarta opciones:
...
> **COMENTARIO:** No todos los organismos tienen un único ensamblado "de referencia". Cuando existen
...
> **IMPORTANTE:** El accession del **ensamblado** (`GCF_000005845.2`) y el accession de la
...
2. Completa la cadena sin consultar todavía ninguna base de datos:
...
1. Intercambia tu cadena con otra persona.
...
3. Corrijan la cadena después de revisar la sección 2 y buscar el organismo en NCBI (Taxonomy o
...
Incorpora la cadena corregida a `doc/protocolo.md`, en una sección `## Estrategia de búsqueda`, y
señala cualquier eslabón que siga pendiente de confirmar.

**Criterio de logro:** la cadena organismo → cepa → ensamblado → versión → registro es completa y
coherente con la pregunta; ningún eslabón se salta ni se inventa.
...
> **ADVERTENCIA:** No todos los ensamblados incluyen los seis archivos. Un ensamblado a nivel
...
mismo ensamblado de referencia.
...
Un cuarto integrante descargó solo `genomic.gff` y after intentó extraer la secuencia de un gen
...
Distingues qué archivo de la familia responde cada tipo de pregunta y reconoces cuándo un archivo por
...
> **IMPORTANTE:** El nombre de la carpeta (`GCF_000005845.2`) no es una elección estética: es el
...
![](images/figura-u3-flujo-recuperacion.png)
*Figura 4. Flujo de recuperación y verificación de integridad de un conjunto de datos. Elaboración propia.*

> **¿SABÍAS QUE?:** Un archivo FASTA truncado a la mitad puede seguir teniendo una línea de
...
```text
Checksum publicado por NCBI para genome.fna:   a1b2c3d4e5f6...
Checksum que tú calculas sobre tu descarga:    a1b2c3d4e5f6...
                                                ¿coinciden? → si sí, la descarga está verificada
```

> **TIP:** NCBI publica un archivo `md5checksums.txt` junto con cada ensamblado, con un checksum por
> cada archivo de la familia. No necesitas calcular nada a mano: solo necesitas comparar.

> **ADVERTENCIA:** Un checksum verifica que el archivo **no cambió entre la fuente y tu copia**. No
...
```text
Archivo: genome.fna
Checksum publicado (NCBI): 9f86d081884c7d659a2feaa0c55ad015
Checksum calculado (local): 9f86d081884c7d659a2feaa0c55ad016
```
...
conjunto de archivos, y ubicas los archivos originales sin alterarlos.
...
- Checksum publicado y resultado de la comparación:
...
- Pregunta biológica: ¿cuántos genes están anotados en el ensamblado de referencia?
- Organismo, cepa o aislamiento: Escherichia coli K-12 MG1655
- Base de datos y colección: NCBI Assembly, RefSeq
- Accession del ensamblado, con versión: GCF_000005845.2
- Accession de la secuencia principal: NC_000913.3
- Archivos recuperados: genome.fna, genomic.gff, genomic.gbff, md5checksums.txt
- Checksum publicado y resultado de la comparación: pendiente de confirmar (se verifica en S9)
- Fecha de consulta y descarga: pendiente de confirmar
- Licencia o condiciones de uso: pendiente de confirmar
- Información no documentada o pendiente de confirmar: fecha exacta de la última actualización de
  anotación
```
...
> **NOTA sobre el Plan operativo:** el Plan de clases 2026 describe la Tarea 4 como "revisión de dos
> bases de datos de NCBI y registro del proyecto". Esta sesión desarrolla esa tarea con dos
> componentes que la satisfacen íntegramente: la comparación de dos bases de datos (Assembly/Genome,
> para los datos de secuencia, y PubMed, para la referencia bibliográfica) en la Práctica 5, y la
> ficha de procedencia como registro del proyecto. Si more adelante el Plan se detalla de otra forma,
> alinea esta tarea a esa versión y anota la discrepancia aquí.
...
> **IMPORTANTE:** Si algún dato no está disponible en la fuente consultada, escribe "no documentado"
> o "pendiente de confirmar". No lo completes por inferencia ni le pidas a una IA que lo invente.

## Evidencia de aprendizaje de S8
...
- 🔴 **Rojo:** elijo un registro sin poder justificarlo, o trato un checksum correcto como prueba de
  que elegí bien el organismo o la versión.
...
> **IMPORTANTE:** Conserva los archivos que recuperaste en esta sesión, sin modificarlos. Los
> necesitarás intactos para S9.

**S9 — Inspección y transferencia verificable de datos biológicos.** Inspeccionarás los archivos sin
modificar los originales, los transferirás entre contextos y demostrarás que origen y destino
contienen los mismos bytes. Esta sesión desarrolla la **Tarea 5**.
