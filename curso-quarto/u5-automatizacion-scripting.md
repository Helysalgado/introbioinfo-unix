...
La Unidad 5 no es un documento único: es esta portada más
...
::: {.callout-note title="nivel real de la unidad"}
*Escribir*, *validar* y *procesar* se ejecutan; *construir una
...
::: {.callout-note title="los títulos del Plan"}
El Plan de clases nombra estas sesiones por su contenido (*Del
...
::: {.callout-important title="el protocolo no se vacía"}
Pierde una función y conserva la más importante. Las
...
::: {.callout-note title="sobre `set -e`"}
Es tentador incluirlo como buena práctica. Se deja fuera a propósito: su
...
::: {.callout-warning}
Pedir un script es lo que mejor hace un asistente, y por eso es donde resulta más
...
::: {.callout-important title="no hay examen práctico en esta unidad"}
El Plan de clases ajustado sustituye el
