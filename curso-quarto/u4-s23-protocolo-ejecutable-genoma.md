...
::: {.callout-important title="qué significa «ejecutable»"}
Un **protocolo ejecutable** es un procedimiento que
...
::: {.callout-tip}
Si has perdido algún archivo intermedio, **no lo repongas todavía**. Descubrir qué falta y
...
En el taller se integra la **ruta crítica** —la
...
::: {.callout-important title="inconsistencias que hay que resolver, no esconder"}
...
::: {.callout-note title="qué inventariar y qué no"}
No transcribas el historial de la terminal. Solo entra en el
...
::: {.callout-important title="el control lo evalúa una persona"}
Aquí nada se detiene solo. El protocolo
...
::: {.callout-note title="qué garantiza `-k2,2` y qué no"}
Agrupa las líneas que comparten valor de cadena, pero el
...
::: {.callout-warning title="el orden lexicográfico de los números"}
Sin `n`, `sort` compara las posiciones como
...
::: {.callout-important title="qué permite y qué no permite esta tabla"}
Permite inspeccionar la organización de
...
::: {.callout-warning}
Copia primero, aparta después y **no borres nada**. Si apartas `data/processed/`
...
::: {.callout-important}
La sección 3 —la tabla de equivalencias— parece burocracia y es lo contrario:
...
::: {.callout-warning}
Un asistente tiende a proponer directamente un script con variables y bucles, que
...
::: {.callout-tip}
Guarda tu protocolo ejecutable tal como quedó hoy. En la unidad siguiente será el punto de
