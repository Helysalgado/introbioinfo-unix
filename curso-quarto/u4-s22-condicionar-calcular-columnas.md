...
::: {.callout-important}
Hoy aparece `awk`, y conviene decir desde el principio qué **no** es esta sesión: no
...
::: {.callout-tip}
Ten a mano el protocolo abierto. Hoy vas a contrastar casi todos los números con alguno
...
::: {.callout-warning title="`-F'\t'` no es opcional"}
Sin él, `awk` separa los campos por **espacios**, y en un
...
::: {.callout-warning title="no sumes longitudes de `gene` y de `CDS` a la vez"}
Un mismo locus genera ambos
...
::: {.callout-tip title="qué debe cumplirse siempre, sea cual sea tu genoma"}
Tus tres comprobaciones a mano tienen
...
::: {.callout-tip title="la mediana, si te interesa"}
La mediana es el valor central: resiste mejor los genes
...
::: {.callout-important title="la longitud del denominador"}
Es la **longitud del replicón**, no la suma de las
...
::: {.callout-important title="comprueba que mediste lo que querías"}
El número de loci que devuelve `grep -F -f`
...
::: {.callout-warning}
Un asistente suele proponer expresiones de `awk` más avanzadas de lo necesario
...
::: {.callout-tip}
Conserva `results/s22/` completo y revisa que cada sección de tu protocolo tenga el comando
