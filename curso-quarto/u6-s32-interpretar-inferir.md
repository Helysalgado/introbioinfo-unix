...
**Antes de clase** lees este módulo y haces un primer intento: mirar
...
::: {.callout-note title="lo que esta sesión NO enseña todavía"}
Homología, ortología, paralogía y transferencia
...
::: {.callout-important title="una fila no es «un hit de una proteína»"}
Si la misma proteína produce dos HSP en
...
::: {.callout-warning}
Un `pident` alto no responde solo tu pregunta biológica. Responde *«¿cuánto
...
::: {.callout-tip}
El `248` no es magia: es la longitud de `ubiE_con.faa`. Si cambias de consulta, cámbialo.
...
::: {.callout-warning title="dos E-values parecidos no equivalen a dos evidencias parecidas"}
Un hit con
...
::: {.callout-important}
«Mejor sustentado» ≠ «primero en la lista». Si tu elección coincide con la fila 1,
...
::: {.callout-important}
Hoy levantamos la prohibición de **interpretar las columnas**. **No** levantamos
...
::: {.callout-important title="nunca solo tablas"}
La tabla sin argumentos es una captura de pantalla con mejor
