...
::: {.callout-important title="hoy no aprendes HPC"}
Aprendes a **ejecutar en otro sitio lo que ya sabes hacer**.
...
::: {.callout-warning title="la cuenta y el acceso se comprueban antes"}
Es la única sesión del curso cuya
...
El taller es enviar, monitorear y recuperar. El núcleo que
...
::: {.callout-tip title="una analogía, con su límite"}
Un clúster se parece a una cocina profesional
...
::: {.callout-warning title="y también te perjudica a ti"}
Un análisis lanzado en el nodo de acceso sigue
...
::: {.callout-tip title="el turno del banco"}
Es la misma idea que un sistema de turnos: no eliges ventanilla
...
::: {.callout-note title="qué NO hace el planificador"}
No revisa tu análisis, no comprueba que tenga sentido, no
...
::: {.callout-important title="y por eso el proyecto tiene que estar en el sitio correcto"}
Si lo dejas en un
...
::: {.callout-note title="el planificador del curso"}
El clúster `chaac` usa **SGE** (*Son of Grid Engine*). Sus
...
::: {.callout-note title="la extensión `.jdl`"}
En este curso los nombramos con la extensión `.jdl`, por convención
...
::: {.callout-note title="las directivas `#$`"}
Las líneas que empiezan por `#$` no son comentarios: son
...
::: {.callout-tip}
Crea el directorio `registros/` **antes** de enviar el trabajo. Si SGE no puede escribir la
...
::: {.callout-important title="el espacio de trabajo"}
En `chaac` el espacio de usuario del curso es
...
::: {.callout-tip}
Es la misma honestidad que aplicaste a los resultados en S28, ahora aplicada a los
...
::: {.callout-warning title="desaparecer no significa haber terminado bien"}
Es exactamente la lección de S24
...
::: {.callout-tip title="el costo oculto"}
Enviar al clúster algo que tarda dos minutos te hace esperar en la
...
::: {.callout-important title="el apartado 6 es el que da sentido a la sesión"}
...
::: {.callout-warning title="por qué este error es distinto de los demás"}
Un *job script* con sintaxis de otro
