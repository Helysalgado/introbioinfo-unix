...
::: {.callout-note}
Esta es la sesión con más casillas cerradas de todo el bloque. Al terminar habrás
...
::: {.callout-important}
Un análisis puede estar completamente libre de falsos positivos y aun así ser
...
::: {.callout-important title="el archivo sabe cosas que tú todavía no sabes"}
Un investigador no solo comprueba
...
::: {.callout-warning title="`uniq` no funciona solo"}
`uniq` compara cada línea con la inmediatamente anterior,
...
::: {.callout-tip}
Sin `-k`, `sort` compara la línea completa empezando por el primer carácter. Sobre una
...
::: {.callout-tip}
Cuatro eslabones son muchos para verificar de una vez. Constrúyela como en S10: ejecuta el
...
::: {.callout-note}
Esto es análisis descriptivo, y es todo lo que necesitas por ahora: describir cómo se
...
::: {.callout-important title="un registro no es un objeto biológico"}
Tu inventario cuenta **líneas de un
...
Dos bases de datos pueden publicar cifras distintas de "genes" para el mismo
...
::: {.callout-tip}
Que la tubería sea idéntica salvo por el número de columna no es casualidad: acabas de
...
::: {.callout-important}
Obtener la misma cantidad por varios caminos **no es redundancia: es validación**.
...
::: {.callout-important}
La independencia de las evidencias tiene **grados**, y las conclusiones se sostienen
...
::: {.callout-note}
Lee la última columna de arriba abajo. Ninguna celda dice "correcto" o "definitivo": todas
...
::: {.callout-important}
Conserva el bloque de S12 tal como está. En S18 volverás sobre estos mismos números
...
::: {.callout-tip}
Guarda `results/s13/` completo y no lo modifiques. En S18 compararás tus conteos de hoy con
