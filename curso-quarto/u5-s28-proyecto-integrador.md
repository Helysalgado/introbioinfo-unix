...
::: {.callout-important title="qué se evalúa hoy y qué no"}
No se evalúa que recuerdes la sintaxis de un comando,
...
::: {.callout-warning title="lo que pasa si algo no está"}
Esta sesión reúne el trabajo de cinco sesiones. Si te
...
El taller es la defensa: ejecutar, comprobar y responder.
...
::: {.callout-warning title="la tentación del último minuto"}
Si con los datos nuevos algo no sale, la reacción
...
::: {.callout-tip}
Empieza por la pregunta biológica, nunca por la herramienta. *«Quería saber cómo se
...
::: {.callout-tip title="no hay una única respuesta correcta"}
Si tomaste otra decisión y puedes justificarla
...
::: {.callout-important title="el apartado 9 es el que distingue un informe de un trabajo científico"}
Cualquiera
...
::: {.callout-note title="cómo se pondera"}
Los ocho elementos no valen lo mismo. Pesan más el protocolo, la
...
::: {.callout-tip title="por qué este prompt sí es buen uso"}
No le pides que haga tu trabajo: le pides que
...
::: {.callout-tip}
Guarda tu proyecto tal como quedó hoy, con su fecha, y consérvalo. Es lo más parecido a un
