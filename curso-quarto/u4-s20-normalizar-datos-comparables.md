...
::: {.callout-important}
Normalizar no significa "hacer que todo coincida". Significa eliminar únicamente
...
::: {.callout-note title="puede que tus listas ya sean comparables"}
Vienen de dos archivos descargados juntos, así
...
::: {.callout-tip}
No corrijas identificadores a mano en un documento. Con tres replicones parece rápido,
...
::: {.callout-warning}
No llames "normalización" a borrar lo que no encaja. Si un identificador no cumple
...
::: {.callout-warning title="las versiones no son decoración"}
`NC_000913.2` y `NC_000913.3` comparten accesión
...
::: {.callout-important}
`sed` no sabe que `chr:` es un prefijo: aplica la regla que escribiste. La validez
...
::: {.callout-tip title="dos controles no admiten interpretación; el tercero sí"}
Sea cual sea tu genoma, la
...
::: {.callout-warning}
"Más coincidencias" no equivale a "mejor resultado". Una transformación puede
...
::: {.callout-important title="no toda tu política es aplicable a una tabla"}
Solo las reglas ancladas al inicio de
...
::: {.callout-tip title="para compartir la tabla"}
Si alguien la necesita en una hoja de cálculo, `tr '\t' ','`
...
::: {.callout-important}
No registres solo la tubería. El comando muestra **cómo** se transformó el texto; la
...
::: {.callout-warning}
Un asistente optimiza para producir una salida de aspecto limpio, y por eso suele
...
::: {.callout-tip}
Conserva `results/s20/` y tu tabla de `data/processed/`. En S21 no partirás de cero:
