...
::: {.callout-important title="qué NO trae esta sesión"}
Ni una sola herramienta de análisis nueva. No hay `grep`
...
::: {.callout-tip}
Si perdiste el respaldo de S23, **no lo repongas ejecutando comandos a mano**. Anótalo como
...
Para que puedas repartir tu tiempo,
...
En el taller se construye y se ejecuta un script
...
La comunidad científica formalizó en 2022 unos principios FAIR específicos para
...
::: {.callout-note title="el `#!` no es un comentario, aunque lo parezca"}
Empieza por `#`, que en shell abre un
...
::: {.callout-tip title="`#!/bin/bash` y `#!/usr/bin/env bash`"}
Verás las dos formas. La segunda busca el
...
::: {.callout-warning title="`chmod 777` nunca"}
Es la receta que más circula por internet y la que más daño
...
::: {.callout-tip}
Escribe el encabezado **antes** que los comandos, no después. Si al redactarlo no puedes
...
::: {.callout-warning title="el error frecuente número uno de esta unidad"}
Al ver que los comandos se han
...
::: {.callout-important title="sigue haciendo falta lo que aprendiste en S23"}
Recuerda las dos clases de entrada:
...
::: {.callout-warning}
Antes de ejecutar por primera vez un script que escribe archivos, lee todas sus
...
::: {.callout-tip title="de dónde salen estos controles"}
No los inventes: son los de la tabla de puntos de
...
::: {.callout-note}
Trabaja sobre una **copia**: `cp src/analizar-genoma.sh /tmp/prueba.sh`. Estos
...
::: {.callout-important title="el punto 5 no es una confesión de trabajo incompleto"}
Es el apartado que hace
...
::: {.callout-warning}
Un asistente tiende a devolver la versión «profesional» del script, con validación,
...
::: {.callout-tip}
Guarda tu script tal como quedó hoy, con su fecha. En S25 no lo tirarás para escribir otro:
