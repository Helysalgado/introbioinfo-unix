...
Este documento se lee **antes de la sesión S2**. Traes hecho el primer
...
La estructura que construyas hoy en tu equipo es la misma que levantarás
...
::: {.callout-note title="dónde encaja el cierre de la unidad"}
El quiz, el reto final y el semáforo de salida están
...
::: {.callout-important}
FAIR **no se logra al final**, al publicar: **empieza en la obtención**. Si no
...
::: {.callout-note}
FAIR **no** equivale a "datos abiertos o gratuitos": significa que, con los permisos que
...
::: {.callout-tip}
Los principios FAIR se van logrando **con la forma en que ejecutas** las fases de
...
::: {.callout-tip}
El **checksum** es una "huella digital" del archivo (un código que cambia si el archivo se
...
::: {.callout-note title="Metadatos de software"}
Además de los datos, el **software** también se documenta. En
...
::: {.callout-note}
Se agrupan todos los datos bajo `data/` y se separan los **originales**
...
::: {.callout-important}
La idea no es "no tocar los datos" en abstracto, sino que **el punto de partida
...
::: {.callout-note}
En esta unidad **no** necesitas crear esta estructura con comandos de Unix. Eso se
...
::: {.callout-note}
Esta sección es el **inicio del eje de IA en espiral** del curso: aquí sentamos las
...
::: {.callout-important}
Un mejor prompt **no sustituye** la validación independiente. Aunque la respuesta
